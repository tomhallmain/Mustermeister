...
# Lets an LLM answer questions about a user's tasks via bounded read-only tool
# calls. The tool bodies themselves (and their exclusion/scoping logic) live
# in TaskToolsService, shared with the external Api::ToolsController - this
# class owns the tool-calling loop, prompt-building, and response parsing
# around them, delegating actual tool execution via run_tool.
class TaskInsightsChatService
  ...
  def initialize(user:, locale:, model_name: nil, excluded_project_ids: [], transcript_messages: [], progress: nil)
    ...
    @locale = locale
    @progress = progress
    @tools = TaskToolsService.new(user: user, excluded_project_ids: excluded_project_ids)
  ...
  def build_prompt(transcript)
    ...
    cap = TaskToolsService::MAX_LIST_ITEMS
    <<~PROMPT
      <CONTEXT>
      ...
      <TOOLS>
      Available tools:
      #{TaskToolsService::TOOL_DEFINITIONS.map { |t| "- #{t[:name]}: #{t[:description]} args=#{t[:args].to_json}" }.join("\n")}
      </TOOLS>
  ...
  def normalized_tool_call(tool_name, arguments)
    tool = tool_name.to_s
    unless tool.present? && TaskToolsService::TOOL_NAMES.include?(tool)
  ...
  def map_external_tool_call(tool, arguments)
    ...
    when "get_tasks", "list_tasks", "fetch_tasks"
      ...
      if keyword.present?
        limit = normalized["limit"] || TaskToolsService::MAX_LIST_ITEMS
        return {
          "type" => "tool_call",
          "tool" => "search_tasks",
      ...
      { "type" => "tool_call", "tool" => "recent_tasks", "arguments" => { "limit" => TaskToolsService::MAX_LIST_ITEMS } }
    else
      nil
    end
  end
  ...
  # Tool bodies live in TaskToolsService (run_tool above delegates to it).
end
